...
@admin.action(description="Архивировать запись")
def mark_archived(modeladmin: admin.ModelAdmin, request: HttpRequest,
                  queryset: QuerySet):
    queryset.update(archived=True)


@admin.action(description="Разархивировать запись")
def unmark_archived(modeladmin: admin.ModelAdmin, request: HttpRequest,
                    queryset: QuerySet):
...
class ProductAdmin(admin.ModelAdmin, ExportAsCSVMixin):
    ...
    fieldsets = [
        ...
        ("Архивация", {
            "fields": ("archived",),
            "classes": ("collapse",),
            "description": "Доп. опция. Архивирует запись"
        }),
        ("Изображения", {
            "fields": ("preview",)
        }),
